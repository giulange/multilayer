...
proj.description    = 'test:SOILCONSWEB on localhost';
...
%                   simulation. It is a switch by which all crops are
%                   deactivated independent from any other configuration
%                   parameter. This is true during the whole simulation
%                   time.
%                   	*0: bare soil (no crop)     --> in [0, W.tmax]
%                       *1: presence of crop        --> in [0, W.tmax]
W.iveg              = 1;
% save:             Flag to set saving or not of current simulation
%                   outputs.
W.save              = 0;
%%   GENERAL | <== from W.*
%%   SOIL WATER 
...
% #SoilLay#             1        2       3      4       5       6       7
%--------------------------------------------------------------------------
% See functions.for,prhead function, lines:532-541
% dap:              (Dry? --> see Variables.for:369) Bulk density [g cm-3]
W.dap               = [ 1.1,     1.1,    1.1     ];
% ------------------
...
%                       *6  --> modified Van Genuchten [when ifr=4]
W.ifc               = [ 1,       1,      1       ];
% ------------------
...
%                       *one    same potential along whole soil grid
%                       *more   potential (linearly) varies with depth
%                   -----------------------------------
%                       Depth [cm]      W.hin [?]
%                       [0,botlim]      [-100,+100]
W.hin               = [
...
% B.top.eto       = [ 0.961	0.361	0.361	1.325	0.314	0.489	0.489	0.472	0.489	0.489	0.911	0.489	0.489	0.492	0.389	0.417	0.417	0.407	0.407	0.647	0.647	0.833	0.833	0.8	0.69	0.69	0.685	0.685	0.518	0.518	0.5	0.858	0.858	0.661	0.661	0.638	0.638	0.6	0.578	0.578	0.623	0.623	0.516	0.516	0.6	0.52	0.52	0.451	0.451	0.443	0.443	0.4	0.4	0.4	0.526	0.526	0.463	0.463	0.4	0.526	0.526	0.611	0.611	0.6	0.6	0.6	0.402	0.402	0.598	0.598	0.593	0.593	0.6	0.637	0.637	0	0.527	0.527	0.5	0.5	0.575	0.575	0.55	0.499	0.499	0.5	0.5	0.5	0.5	0.499	0.499	0.5	0.5	0.508	0.508	0.5	0.462	0.462	0.5	0.5	0.518	0.518	0.5	0.426	0.426	0.5	0.5	0.463	0.463	0.5	0.437	0.437	0.4	0.4	0.401	0.401	0.4	0.4	0.4	0.4	0.4	0.568	0.568	0.4	0.405	0.405 ];
% B.top.eto       = B.top.eto(1:120);
B.top.radi      = meteo_in.rad_int(1:end)';%        []

% ---CORRECT!!!
B.top.rain      = 0.1*meteo_in.rain_cum(1:end)';%   [cm d-1]
warning('Rain < 2 mm is set to 0 mm')
B.top.rain( B.top.rain<0.2 ) = 0;
...
% clean
clear curs conn driver db_url db_name driver iCol iRow table_res query_str% meteo_in
%%   TOP BOUNDARY 
% ----------------------------------

B.top.description   = 'Preliminary trials on SOILCONSWEB';%--> 'a discretization...'
% ------------------
% cfbs:             Coefficient (-) to convert potential evapotranspiration
%                   into potential evaporation.
%                   See Swap-32 manual, §3.7.3, page 67.
%                   I was trying to implement V.Ke following FAO paper 56,
%                   chapter 7, page 142 of pdf. V.Ke is in the CROP file.
B.top.cfbs          = 1.00;
% ------------------
% revameth:         Flag to set the method("meth") of reduction("r") of
%                   potential soil evaporation("eva") on daily basis.
%                   Same as "swredu" flag used in SWAP.
...
% ------------------
% should be defined for each single fertilizer/compound supplying date!!!!
B.Ctop.dL           = 30;   % [cm]
% ------------------
...
%                   (sold by vendors). This option is not implemented yet
%                   (try to fill the code in the
%                   multilayer_check_and_load.m fucntion.
%                   isfert = true
...
% IMPORTANT NOTES:
%-1-Units –––> [mg cm-x] or [mg cm-x dt-1] ????? I think the latter for
%              fertigation and former for fertilization.
% 
%-2-Antonio says that fertigation should be set from here. My opinion is
%   that fertigation should be placed in crop/V.irri. I think we have to
%   remove fertigation from here for the following reasons:
%       1.\ fertigation is given during crop, so is has sense to configure
%           the parameter in crop file;
%       2.\ fertigation is given together with irrigation water and – also
%           in the case in which small amount of water is used – this water
%           must be included in water transport and balance. This is not
%           possible configuring fertigation in S.compounds
%       3.\ fertigation is a rate variable and inorganic fertilization is a
%           state variable.
S.compounds         = {
%      ----------------------------------------------------------------------------
%      __YYYY-MM-DD,HH__    _____________[mg cm-2]_______________  _[mg cm-3 day-1 H2O]_ 
%                           _UREA_  ___organic___   ____solid____  __fertigation________   
...
% rotation:         Crop rotation scheme during the simulation period.
%                   User has to define crop parameters in a file (CROPFIL).
%                   -----------------------------------
V.rotation          = {
%   CROPSTART       CROPEND         CROPNAME    CROPFIL         
%   [day]           [day]           [char]      [char]          
%   [sdate,edate]   [sdate,edate]   [0,1d03]    [0,3]           
%   '2025-01-01'    NaN             NaN         NaN             
    '2013-04-01'    '2013-09-01'    'mais'      'mais_FAO900.m' 
%     '2013-09-01'    '2014-03-01'    'loietto'   'loietto.m'           
%   '2014-03-01'    '2016-08-22'    'medica'    'medica.m'    
                    };
%                   -----------------------------------
%%   SOLUTE Jury -- translate definitions in variables that can be used by the program
% ----------------------------------
if W.isol==1
...
% kf2.
% See Eq. 8.9 in SWAP manual, page 176.
% 
% NX.kf1:           Freundlich (adsorption?) coefficient.
%                   [0..100 cm3 g-1]
%                      'NH'   'NO'
S.CDE.NX.Kf1        = [0.100, 0.000];
% 
% NX.kf2:           Freundlich exponent. It is called FREXP in SWAP.
%                   [0..10 -]
%                      'NH'   'NO'
S.CDE.NX.Kf2        = [1.000, 1.000];

% NX.Kr:            Fattore di attingimento radicale.
%                   It accounts for positive or negative selection of
%                   solute ions relative to the amount of soil water that
%                   is extracted. See Eq. 8.10 SWAP manual.
%                   [0..10 -]
%                      'NH'   'NO'
...
% lambda:           Dispersion length, defined at different depths.
%                   It is defined as LDIS in SWAP (see Eq. 8.5).
%                   --------------------------------------------
