...
hgrad = NaN(P.nz,1);
hgrad(2:P.nz) = (P.h(1:P.nz-1) - P.h(2:P.nz)) ./ (0.5*(P.nodes.dz(1:P.nz-1) + P.nodes.dz(2:P.nz))) +1;
%% *top          node
% [headcalc.for, lines 175-180]
if ~isflux%     HEAD controlled top boundary
    % Fi-function:
    Fi_topB     = - P.Kim2(1).*((hsurf-P.h(1))./(0.5*P.nodes.dz(1)) +1 );
...
Fi(1)   = P.nodes.dz(1)./P.dt.*(P.teta(1) - P.teta_jm1(1))*P.FrArMtrx(1) + ...
        ...
        + P.Kip2(1) .* hgrad(2);
%% *intermediate nodes
Fi(2:P.nz-1) = P.nodes.dz(2:P.nz-1)./P.dt.*(P.teta(2:P.nz-1) - P.teta_jm1(2:P.nz-1)).*P.FrArMtrx(2:P.nz-1) + ...
...
Fi(P.nz)= P.nodes.dz(1)./P.dt.*(P.teta(P.nz) - P.teta_jm1(P.nz))*P.FrArMtrx(P.nz) + ...
        + P.sink(P.nz) + macr(P.nz) + ...
